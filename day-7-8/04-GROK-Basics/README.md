...
# **11. Understanding `_grokparsefailure`**

When Grok fails to match:

```
"tags": ["_grokparsefailure"]
```

## **Common reasons Grok fails:**

* Missing escape characters (`\"` instead of `"`)
* Wrong spacing
* Wrong pattern order
* Not using `DATA` or `GREEDYDATA` for variable fields
* Trying to match numeric fields with `%{WORD}`
...
```
sudo journalctl -u logstash -f | grep grok
```
...
# **12. Grok Best Practices**

### ✔ Start simple

Use `%{DATA:field}` first, refine later.

### ✔ Avoid `GREEDYDATA` unless necessary

It can swallow everything.

### ✔ Use multiple patterns for multiple log formats
...
### ✔ Make heavy use of test tools

Always validate patterns before production.

---

# **13. Dissect vs Grok (Important Real‑World Note)**

Grok = regex (slower, more powerful)
Dissect = delimiter-based (faster, simpler)
...
Use Dissect when log structure is predictable.
Use Grok when logs vary or require regex.
